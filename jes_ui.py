...
class UI:
    def __init__(self, _W_W, _W_H, _MOVIE_SINGLE_DIM, _GRAPH_COOR, _SAC_COOR, _GENEALOGY_COOR,
        ...
        # Scaling factor based on reference resolution (1920x1080)
        self.scale = 1300 / 1920  # ≈ 0.6770833
        self.reference_font_sizes = {'big': 60, 'small': 30, 'tiny': 21}
        self.bigFont = pygame.font.Font('C:/Users/RIC0016/AppData/Local/Microsoft/Windows/Fonts/Overpass-Regular.ttf', int(self.reference_font_sizes['big'] * self.scale))
        self.smallFont = pygame.font.Font('C:/Users/RIC0016/AppData/Local/Microsoft/Windows/Fonts/Overpass-Regular.ttf', int(self.reference_font_sizes['small'] * self.scale))
        self.tinyFont = pygame.font.Font('C:/Users/RIC0016/AppData/Local/Microsoft/Windows/Fonts/Overpass-Regular.ttf', int(self.reference_font_sizes['tiny'] * self.scale))

        # Load and scale background image
        self.BACKGROUND_PIC = pygame.transform.scale(
            pygame.image.load('C:/Users/RIC0016/OneDrive - Templestowe College/Desktop/Python/jes/visuals/background.png'),
            (_W_W, _W_H)
        )
        ...
        self.MS_W = self.W_W - self.CM_MARGIN1 * 2
        self.MS_WC = self.MS_W - self.INFO_W - self.COLUMN_MARGIN
        ...
        # Scale preview locations
        self.previewLocations = [
            [int(570 * self.scale), int(105 * self.scale), int(250 * self.scale), int(250 * self.scale)],
            [int(570 * self.scale), int(365 * self.scale), int(250 * self.scale), int(250 * self.scale)],
            [int(570 * self.scale), int(625 * self.scale), int(250 * self.scale), int(250 * self.scale)]
        ]
        ...
        self.storage_coor = (int(660 * self.scale), int(52 * self.scale))
        self.running = True
    ...
    def addButtonsAndSliders(self):
        self.genSlider = Slider(
            ...
            (
                int(40 * self.scale),
                int(self.W_H - 100 * self.scale),
                int(self.W_W - 80 * self.scale),
                int(60 * self.scale),
                int(140 * self.scale)
            ),
            0, 0, 0, True, True, self.updateGenSlider
        )
        
        buttonCoor = []
        for i in range(6):
            buttonCoor.append((
                int(self.W_W - 1340 * self.scale + 220 * i * self.scale),
                int(self.W_H - self.MENU_TEXT_UP),
                int(200 * self.scale),
                int(60 * self.scale)
            ))
    ...
    def drawInfoBarCreature(self, creature):
        ...
        if creature.fitness is not None:
            ...
            stri = [f"Creature #{creature.IDNumber}", f"Species: {species_to_name(creature.species, self)}",
        ...
        for i in range(len(stri)):
            ...
            centerText(self.infoBarScreen, stri[i], X_center, int(self.MOVIE_SINGLE_DIM[1] + 40 * self.scale + 42 * i * self.scale),
                       color, self.smallFont)
    ...
    def drawMovieGrid(self, screen, coor, mask, titles, colors, font):
        ...
        for i in range(LMS):
            ...
            if titles is not None:
                centerText(screen, titles[i], x + W / 2, y + H - int(30 * self.scale), colors[i], font)
    ...
    def drawInfoBarSpecies(self, species):
        ...
        if species in self.sim.species_pops[now]:
            ...
            extinct_string = ""
        strings = [f"Species {s_name}", f"Ancestor {a_name}",
        ...
        for i in range(len(strings)):
            ...
            centerText(self.infoBarScreen, strings[i], X_center, int(self.MOVIE_SINGLE_DIM[1] + 40 * self.scale + 42 * i * self.scale),
                       colors[i], self.smallFont)
        
        self.drawLightboard(self.infoBarScreen, species, now,
                            (self.INFO_W * 0.6, self.MOVIE_SINGLE_DIM[1] + int(10 * self.scale),
                             self.INFO_W * 0.37, self.MS_H - self.MOVIE_SINGLE_DIM[1] - int(20 * self.scale)))
    ...
    def drawMenuText(self):
        ...
        self.screen.blit(titleSurface, (int(40 * self.scale), int(20 * self.scale)))
        ...
        self.screen.blit(genSurface, (int(40 * self.scale), y))
    ...
    def drawPreviews(self):
        ...
        if gen >= 0 and gen < len(self.sim.rankings):
            ...
            for r in range(3):
                ...
                centerText(preview, f"{names[r]} creature", DIM[0] / 2, DIM[1] - int(20 * self.scale), self.WHITE, self.smallFont)
                alignText(preview, dist_to_text(creature.fitness, True, self.sim.UNITS_PER_METER), int(10 * self.scale),
                          int(20 * self.scale), self.WHITE, self.smallFont, 0.0, None)
